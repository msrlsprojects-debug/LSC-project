...
export default function UserManagementPage() {
  ...
  const loadUsers = async () => {
    ...
    const res = await fetch('/api/block/users', {
      headers: {
  ...
  const resetPassword = async (user_id: string) => {
    ...
    return 0; // Remove this line after testing to prevent actual API calls during development
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <Link
          href="/dashboard/block/users/create"
        ...
        <table className="w-full text-sm">
          <thead className="bg-slate-50 border-b border-slate-200">
            <tr>
              ...
              {/* <th className="px-6 py-4 text-left font-semibold text-slate-700">LSCs</th> */}
            ...
            {loading ? (
              ...
              users.map((u) => (
                <tr key={u.user_id} className="hover:bg-slate-50 transition-colors">
                  ...
                  {/* <td className="px-6 py-4 text-slate-600">lscs</td> */}
                  <td className="px-6 py-4 text-right space-x-4">
                    {resetFor === u.user_id ? (
                      ...
                      <>
                        {/* <Link
                          href={`/dashboard/block/users/anchor/${u.user_id}`}
